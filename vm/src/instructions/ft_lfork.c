...
/*   Updated: 2019/11/13 11:34:36 by anjansse         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static inline t_process	*process_copy(t_cw *cw, t_process *copy_cp, int16_t pc)
{
	t_process		*cp;
	int8_t			i;
...
void					ft_lfork(t_cw *cw, t_process *cp)
{
	...
	if (cp->param_type[0] == T_DIR)
	{
		...
		FLAG & FL_VER4 ? printf("P%5d | lfork %d ", P_I, address) : 0;
		address += cp->pc;
		if (address < 0)
			address += MEM_SIZE;
		else
