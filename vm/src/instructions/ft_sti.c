...
/*   Updated: 2019/11/13 10:05:58 by anjansse         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void						ft_sti(t_cw *cw, t_process *cp)
{
	...
	FLAG & FL_VER4 ? printf("sti r%d ", cp->param_value[0]) : 0;
	param1 = param_1(cw, cp);
	param2 = param_2(cw, cp);
	FLAG & FL_VER4 ? printf("       | ") : 0;
	FLAG & FL_VER4 ? printf("-> store to %d + %d", param1, param2) : 0;
	FLAG & FL_VER4 ? printf(" = %d ", param1 + param2) : 0;
	offset = pc_idx_mod(cp, param1 + param2);
	FLAG & FL_VER4 ? printf("(with pc and mod %d)\n", offset) : 0;
